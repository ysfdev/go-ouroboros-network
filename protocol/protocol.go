...
type MessageHandlerFunc func(uint, []byte) error
type MessageFromCborFunc func(uint, []byte) (interface{}, error)
...
func (p *Protocol) recvLoop() {
	...
	for {
		...
		if p.msgFromCborFunc != nil {
			msg, err := p.msgFromCborFunc(msgType, p.recvBuffer.Bytes())
			if err != nil {
				p.errorChan <- err
			}
			if msg == nil {
				p.errorChan <- fmt.Errorf("%s: received unknown message type: %#v", tmpMsg)
			}
			fmt.Printf("msg = %#v\n", msg)
		}
		// TODO: replace with above
		err = p.msgHandlerFunc(msgType, p.recvBuffer.Bytes())
		if err != nil {
			p.errorChan <- err
		}
